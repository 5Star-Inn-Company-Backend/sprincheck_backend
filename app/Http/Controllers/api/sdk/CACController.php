...
//            $file = $request->file('file');
    ...
    public function byname(Request $request)
    {
        ...
        $fee= (new \App\Models\TransactionFee)->getTransactionFee($biz->id,"CAC_BY_NAME");

        if($fee > $biz->wallet){
            return response()->json(['success' => 0, 'message' => "It cannot be processed. Check your wallet balance"]);
        }

        Log::info("Running CAC SEARCH on ".$input['name']);

        try {
            $userService = new MonoService();
            $res=$userService->cacName($input['name'],$biz->id,"API");
            $kyc=$res['kyc'];
            $data=$res['data'];
...
//        }
        ...
        ServiceDebitJob::dispatch($fee, $reference,$biz, 'CAC_SEARCH');

        return response()->json(['success' => 1, 'message' => 'Search Successful', 'data' => $data]);
    }
    ...
    public function shareHolders(Request $request)
    {
        ...
        $fee= (new \App\Models\TransactionFee)->getTransactionFee($biz->id,"CAC_SHAREHOLDERS");

        if($fee > $biz->wallet){
            return response()->json(['success' => 0, 'message' => "It cannot be processed. Check your wallet balance"]);
        }

        Log::info("Running CAC_SHAREHOLDERS on ".$input['biz_id']);

        try {
            $userService = new MonoService();
            $res=$userService->cacShareHolders($input['biz_id'],$biz->id,"API");
            $kyc=$res['kyc'];
            $data=$res['data'];
...
//        }
        ...
        ServiceDebitJob::dispatch($fee, $reference,$biz, 'CAC SHAREHOLDERS');
